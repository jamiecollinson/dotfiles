...
God mode is a neat alternative to evil mode. You might think of it as a non-editing state in which you use emacs keys without the modifiers.

Unlike vim it doesn't have the concept of /motions/, but perhaps it fits better with the rest of the emacs ecosystem. Currently trying it out.
...
=notmuch= is a very fast mail client. It needs to be installed externally, e.g. with =brew install notmuch= but can then be used within emacs.
...
Sometimes I need to explore REST services, why not use Emacs?
